...
/// relative to its own baseline, the root subtree in [`Self::root`] and the others in
/// [`LineBoxBuffers::subtrees`]; the subtrees are only positioned against each other once the
...
impl LineBoxMetrics {
    ...
    fn add_strut(&mut self, strut: &StyleMetrics, buffers: &mut LineBoxBuffers) {
    ...
    fn subtree_mut<'a>(
        &'a mut self,
        root: u16,
        buffers: &'a mut LineBoxBuffers,
    ) -> &'a mut SubtreeExtents {
        if root == 0 {
            return &mut self.root;
        }
        let index = match buffers.subtrees.iter().position(|s| s.root == root) {
            Some(index) => index,
            None => {
                buffers.subtrees.push(SubtreeExtents::new(root));
                buffers.subtrees.len() - 1
            }
        };
        &mut buffers.subtrees[index]
    }
    ...
    fn add_style(
        ...
        while !buffers.contributed.contains(&index) {
            ...
            let subtree = self.subtree_mut(metrics.aligned_subtree, buffers);
            subtree
                .line_box
    ...
    fn add_text(
        ...
        let subtree = self.subtree_mut(aligned_subtree, buffers);
        subtree
            .line_box
