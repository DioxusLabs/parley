                    ...
                    for atom in slice.atoms_from(self.state.cluster_idx) {
                        ...
                        // we fall back to appending the content to the line anyway.
                            ...
                            if is_space && style.text_wrap_mode == TextWrapMode::Wrap {
                                ...
                                // run boundaries), trailing spaces at the end of the layout do
                                // not start an empty line, and a following newline breaks the
                                // line itself. An inline box following the space still starts
                                // a new line.
                                ...
                                let keep_hanging = match next_is_text {
                                    ...
                                    Some(true) => self
                                        ...
                                        .is_some_and(|next| {
                                            whitespace_can_hang(next.info.whitespace())
                                        }),
                                };
                                if keep_hanging {
                                    continue;
