...
use parley::{
    ...
    InlineBoxKind, Layout, OverflowWrap, StyleProperty, TextWrapMode, WhiteSpaceCollapse,
    WordBreak,
};
...
fn break_spaces_preserves_source_across_spans() {
        ...
        .chain([source.len()])
    {
        ...
        for line in layout.lines() {
            close(line.metrics().hanging_advance, 0.);
        }
    }
}
...
fn break_spaces_wraps_each_space_without_hanging() {
    ...
    for text in ["a   b", "    "] {
        for width in [0., space + 0.1] {
            ...
            close(widths.max, advance(&mut env, text));
        }
    }
}
    ...
    for separator in [" ", "\t", "\u{3000}"] {
        ...
        for ending in ["", "\nb", "\r\nb", "\u{2028}b", "\u{2029}b"] {
            ...
            for wrap in [TextWrapMode::Wrap, TextWrapMode::NoWrap] {
                ...
                close(layout.calculate_content_widths().max, full);
                layout.align(Alignment::Right, AlignmentOptions::default());
                close(layout.get(0).unwrap().metrics().offset, 20.);
                layout.align(Alignment::Center, AlignmentOptions::default());
                close(layout.get(0).unwrap().metrics().offset, 10.);
            }
        }
    }
}
...
fn break_spaces_ideographic_space_and_nonbreaking_separators() {
    ...
    for word_break in [WordBreak::Normal, WordBreak::BreakAll, WordBreak::KeepAll] {
        ...
        close(first_line.metrics().hanging_advance, 0.);
        layout.break_all_lines(Some(0.));
        close(layout.full_width(), layout.calculate_content_widths().min);
    }
    for separator in ["\u{a0}", "\u{2007}", "\u{202f}"] {
        ...
        assert_eq!(layout.len(), 1);
        close(layout.get(0).unwrap().metrics().hanging_advance, 0.);
        close(layout.calculate_content_widths().min, layout.full_width());
    }
}
